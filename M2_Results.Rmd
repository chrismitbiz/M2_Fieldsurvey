...
```{r phylodiv, echo=FALSE, fig.width=7.5, fig.asp=0.35, message=FALSE, warning=FALSE}
p1 <- data.frame(sample_data(physeqB)) %>% ggboxplot(., x = "Dloss_fct2", y = "PD", add = c("jitter"), scales = "free",fill = "Soil", ylab = "Faith PD", xlab = "Dieldrin loss" ) + stat_compare_means(comparisons = my_comparisons,label = "p.signif", label.y = c(100)) + stat_compare_means( method = "kruskal",label = "p.signif", label.y = c(110)) + theme_bw() 

p2 <- ord_pcoa <- phyloseq::ordinate(rare_Bac, "PCoA", "unifrac",weighted=FALSE)
p.pcoa2 <- phyloseq::plot_ordination(rare_Bac, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size =D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) +  scale_colour_gradient(low = "cadetblue1", high = "navy")+ guides(alpha = F) + annotate(geom = 'text', label = "unweighted unifrac", x = -0.15, y = -Inf, hjust = -0.05, vjust = -1)

ggarrange(p1, p.pcoa2)
