...
p.pcoa3 <- phyloseq::plot_ordination(physeq.B.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Bacteria") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)") 
...
p.pcoaF3 <- phyloseq::plot_ordination(physeq.F.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Fungi") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)") 
...
p.pcoaE3 <- phyloseq::plot_ordination(physeq.E.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size = D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) +  ggtitle("Enzyme metagenome")+  scale_colour_gradient(low = "cadetblue1", high = "navy")+ guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)") 
...
print(paste("After filtering the analysis included", ntaxa(physeq.B.flt),"and", ntaxa(physeq.F.flt),"bacterial and fungal taxa in the Kurosol and ", ntaxa(physeq.E.flt), "enzyme encoding genes"))

```

### Test for differences in compositional variance (Betadisp, Bray-Curtis)
...
dis <- vegdist(otus)
groups <- data.frame(Soil = sample_data(physeq.B.flt)$Soil)
...
dis <- vegdist(otus)
groups <- data.frame(Soil = sample_data(physeq.F.flt)$Soil)
betad <- betadisper(dis, groups$Soil)
avobetad <- anova(betad)
...
dis <- vegdist(otus)
groups <- data.frame(Soil = sample_data(physeq.E.flt)$Soil)
betad <- betadisper(dis, groups$Soil)
avobetad <- anova(betad)
...
kable(betatest,caption = "Test for differences in compositional variance between the Kurosol and the Chromosol. All compositions were filtered to the most common features that were present in both soils.")
```
...
Figure \@ref(fig:pcoa) compares the compositions of the most common bacteria (a), the gene abundances as predicted from bacterial ASVs with Picrust2 (b) and fungi (c). It only included taxa and genes that were commonly found in both soils and included 175 bacterial and 65 fungal taxa as well as 1988 enzyme encoding genes. It showed that the Kurosol contained a different composition of key taxa with greater dispersion (betadisp *p* < 0.05).
