---
title: "PreDRAFT: A preliminary field survey indicated that carbon quality is a driver for the metabolic capacity of soils to biodegrade persistant organic pollutants"
...
# Summary of results  
- The results show that carbon quality was related to functional diversity and associated to dieldrin dissipation. For example, genetic potentials of lactose, galactose, fucose, rhamnose and glycole degradation predicted high dieldrin dissipation and concurrently greater humus content.
...
>>>>>>> edd0d947fd5012b25028ce02147136ac427f19f2
...
Overall the two soils contained a different carbon quantity and quality. Figure \@ref(fig:carbonquality) showed that the Kurosol, the soil with greater long-term dieldrin losses, contained significantly less total carbon (TC). On the other hand, the organic matter composition of the Kurosol was composed of more humus (HUM) and particulate organic carbon (POC) relative to the resistant organic carbon (ROC). And a higher dieldrin loss in the Kurosol was associated with more humus. A higher proportion of POC was associated with more dieldrin loss. Furthermore, greater long-term dieldrin losses coincided with a low carbon-to-nitrogen ratio (C:N) and a high hydrogen-to-carbon ratio (H:C). Correlation analysis further highlighted that greater dieldrin losses were correlated to low C:N, low ROC and a high POC.

<br/>

## Compositional analysis
```{r pcoa, echo= FALSE,  fig.cap="Ordinations of Aitchison distances were compared for compositions of the most common bacteria, its enzyme metagenome as predicted by Picrust2 and fungi (n = 33). After filtering the analysis included 175 and 65 bacterial and fungal taxa in the Kurosol respectively and 1988 enzyme encoding genes which were present in both soils. Permanova results based on Bray-Curtis dissimilarities are shown. Symbols represent samples and their proximity to each other indicates compositional similarity. The colour and size of symbols indicate the long-term dieldrin loss (%) and dieldrin concentrations respectively. The shape of symbols indicates sample grouping into four factors where the loss is either below or above the median dieldrin loss in each soil type (Median loss was 43% and 72% in the Chromosol and Kurosol respectively).", fig.width=9, fig.asp= 0.4, message=FALSE, warning=FALSE, results=FALSE}

physeq.B.flt  <-  prune_samples(sample_data(physeqB)$Farm != "C", physeqB)
physeq.B.flt = filter_taxa(physeq.B.flt, function(x) sum(x > 0) > (0.8*length(x)), TRUE) #must be present in x percent of samples
...
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.60*length(x)), TRUE) #must be present in x percent of samples
...
physeq.E.flt = filter_taxa(physeq.E.flt, function(x) sum(x > 0) > (0.8*length(x)), TRUE) #must be present in x percent of samples
...
p.pcoa3 <- phyloseq::plot_ordination(physeq.B.flt, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Bacteria") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = bquote('Dieldrin (µg '*~g^-1*')'), color = "Dieldrin loss (%)", shape = "") + annotate("text", x = -2, y = 3.25,label = paste(expression(atop('Permanova (Bray)', italic(R)^2~ '=' ~ '0.43,' ~  italic(p) ~ '=' ~ 0.001))), parse = TRUE, size = 3) 
...
p.pcoaF3 <- phyloseq::plot_ordination(physeq.F.flt, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Fungi") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)", shape = "") + annotate("text", x = 1, y = -3,label = paste(expression(atop('Permanova (Bray)', italic(R)^2~ '=' ~ '0.29,' ~  italic(p) ~ '=' ~ 0.001))), parse = TRUE, size = 3)
...
#permutest(ord_ccaB)
#anova(ord_ccaB)
#anova(ord_ccaB0, ord_ccaB4) #POC contributing the least
#anova(ord_ccaB0, ord_ccaB5) #HUM contributing the most
#anova(ord_ccaB0, ord_ccaB6)
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss") + geom_point(aes(size =  HUM.pct)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Bacteria, Kurosol") + labs(size= "Humus (%)", color = "Dieldrin loss (%)") + scale_size(breaks = c(42,44,46,48,50)) + annotate("text", x = 5, y = 12,label = paste(expression('(F'[3*','*14]~'='~2.4*', '*italic(p)~'= 0.01)')), parse = TRUE, size = 3)
...
physeq.F.flt  <-   prune_samples(sample_data(physeq.F.flt)$Farm != "C", physeq.F.flt)
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.15*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt <-  microbiome::transform(physeq.F.flt, "clr") #clr transform

ord_ccaF <- phyloseq::ordinate(physeq.F.flt, "RDA", "bray",formula = ~ HUM.clr + POC.clr + ROC.clr)

#Testing significance
#ord_ccaF0 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ 0)
#ord_ccaF1 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ HUM.clr + ROC.clr)
#ord_ccaF2 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ POC.clr + ROC.clr)
#ord_ccaF3 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ POC.clr + HUM.clr)
#ord_ccaF4 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ POC.clr)
#ord_ccaF5 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ HUM.clr)
#ord_ccaF6 <- phyloseq::ordinate(physeq.B.flt, "RDA", "bray",formula = ~ ROC.clr)
#require(vegan)
#permutest(ord_ccaF)
#anova(ord_ccaB0, ord_ccaB4) # POC the least
#anova(ord_ccaB0, ord_ccaB5) # HUM contributing most 
#anova(ord_ccaB0, ord_ccaB6) 



p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss") +
    geom_point(aes(size =  HUM.pct)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Fungi, Kurosol") + labs(size= "Humus (%)", color = "Dieldrin loss (%)") + scale_size(breaks = c(42,44,46,48,50)) + annotate("text", x = 3.2, y = 8,label = paste(expression('(F'[3*','*14]~'='~2.3*', '*italic(p)~'= 0.01)')), parse = TRUE, size = 3)  
...
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss", shape = "Soil") + geom_point(aes(size = HUM.pct)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Bacteria, Chromosol") + scale_shape_manual(values = c(17)) + labs(size= "Humus (%)", color = "Dieldrin loss (%)") + scale_size(breaks = c(42,44,46,48,50))+ annotate("text", x = -4.8, y = 11,label = paste(expression('(F'[3*','*11]~'='~1.1*', '*italic(p)~'= 0.33)')), parse = TRUE, size = 3) + guides(shape = FALSE)
...
physeq.E.flt  <-   prune_samples(sample_data(physeq.E.flt)$Farm != "C", physeq.E.flt)
physeq.E.flt = filter_taxa(physeq.E.flt, function(x) sum(x > 0) > (0.15*length(x)), TRUE) #must be present in x percent of samples
physeq.E.flt <-  microbiome::transform(physeq.E.flt, "clr") #clr transform
...
#permutest(ord_ccaE)
#Testing significance
#ord_ccaF0 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ 0)
#ord_ccaF1 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ HUM.clr + ROC.clr)
#ord_ccaF2 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ POC.clr + ROC.clr)
#ord_ccaF3 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ POC.clr + HUM.clr)
#ord_ccaF4 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ POC.clr)
#ord_ccaF5 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ HUM.clr)
#ord_ccaF6 <- phyloseq::ordinate(physeq.E.flt, "RDA", "bray",formula = ~ ROC.clr)
#require(vegan)
#permutest(ord_ccaE)
...
p.ccaE  <- phyloseq::plot_ordination(physeq.E.flt, ord_ccaE , color = "Dloss") +
    geom_point(aes(size =  HUM.pct)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Enzyme metagenome, Kurosol") + labs(size= "Humus (%)", color = "Dieldrin loss (%)")+ scale_size(breaks = c(42,44,46,48,50)) + annotate("text", x = 3, y = 4.5,label = paste(expression('(F'[3*','*14]~'='~2.8*', '*italic(p)~'= 0.01)')), parse = TRUE, size = 3)
...
p.ccaE  <- phyloseq::plot_ordination(physeq.E.flt, ord_ccaE , color = "Dloss", shape = "Soil") + geom_point(aes(size = HUM.pct)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Enzyme metagenome, Chromosol") + scale_shape_manual(values = c(17)) + labs(size= "Humus (%)", color = "Dieldrin loss (%)") + scale_size(breaks = c(42,44,46,48,50)) + annotate("text", x = 3, y = 5.1,label = paste(expression('(F'[3*','*11]~'='~0.84*', '*italic(p)~'= 0.85)')), parse = TRUE, size = 3) + guides(shape = FALSE)
...
print(paste("After filtering the analysis included", ntaxa(psKbac),"and", ntaxa(psKfun), "bacterial and fungal taxa in the Kurosol and ", ntaxa(psCbac), "and", ntaxa(psCfun), "in the Chromosol"))
```
  
<br/>
  
Figure \@ref(fig:pcoa) compares the compositions of the most common bacteria (a) as well as gene abundances predicted from bacterial ASVs with Picrust2 (b) and fungi (c). It only included taxa and genes that were commonly found in both soils (175 bacterial taxa, 65 fungal taxa and 1988 enzyme encoding genes). It showed that the Kurosol contained a different microbial composition with greater community and genetic dispersion (betadisp *p* < 0.05 with soil type as grouping variable). 
  
Furthermore, figure \@ref(fig:pcoa) shows that the community and genetic composition was different between 'low-loss' and 'high-loss' groups in the Kurosol (<72 % and >=72 % dieldrin loss) and in the Chromosol (<=43 % and >43 % dieldrin loss). These compositional differences between groups was significant in the Kurosol (Permanova *p* < 0.001) and only marginally significant in the Chromosol (Permanova *p* = 0.05 bacteria, *p* = 0.16 fungi).  
...
Microbial richness and diversity was greater in samples with high long-term dieldrin losses (Fig. S\@ref(fig:alphadloss)a, b and c). Soil diversity was further compared on a per-unit-dieldrin (diversity µg g^-2^) basis (Fig. S\@ref(fig:alphaperD17)). It showed that the Kurosol, which contained less organic matter and dieldrin, had significantly higher diversity for each µg g^-1^ of dieldrin compared to the Chromosol. Hence, there was greater diversity for each molecule of dieldrin in the high-loss environment, including the functional diversity based on the enzyme metagenome.  
...
Chlorosalicylate degradation was the only genetic potential found that was related to xenobiotic chlorinated compound degradation and these metabolic potentials were significantly greater in samples with high dieldrin loss (Fig. S\@ref(fig:xenobgraphs)). Furthermore figure S\@ref(fig:xenobgraphs) shows that the mean metabolic potential of aromatic compound degradation was greater in the Kurosol and greatest in the samples with highest dieldrin losses. Chlorosalicylates are chloroaromatics and active ingredients or additives of antimicrobial products and pesticides (reference).  
...
Figure \@ref(fig:trees) highlights that although the phylogenetic composition of the Kurosol and the Chromosol was similar, the Kurosol contained more unique ASVs in high-loss samples, implying a greater phylogenetic versatility in samples with greater dieldrin degradation. A greater phylogenetic dispersion was also apparent from the Faith PD diversity index,  fig. S\@ref(fig:phylodiv)a). 
  
Moreover, generalised linear models (glm) in the R package 'phylofactor' indicated that there were phylogentic clades that had a significantly higher chance to be found in 'high-loss' samples (shown as polygons on phylogenetic tree in figure  \@ref(fig:trees)a).  
  
For example, clades of Acidobacteria, Actinobacteria, Bacteriodetes, Chloroflexi, Planctomycetes and *Geobacter sp.* were more likely to found in samples with high dieldrin losses than could be explained by chance alone (Fig. \@ref(fig:trees)b). All of these clades contained more and more different ASVs in 'high-loss' samples (**MakeTable**).
...
The association network of bacteria and fungi in the Kurosol (Fig. \@ref(fig:networkmodulesandunique)) shows a clear separation of associations that cluster around ASVs that were unique in the high-loss samples and ASVs that were unique in low-loss samples, indicating those unique ASVs were driven by different environmental conditions and were an integral part in mediating associations. This was not seen in the Chromosol (Fig. \@ref(fig:networkmodulesandunique)). 
