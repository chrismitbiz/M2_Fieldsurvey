...
options(knitr.kable.NA = '')
kable(p1,  format = "html", caption = "Number of ASVs in taxonomic groups which had a higher chance to be present in samples with high dieldrin loss than could be explained by chance (n = 33)", digits = 2 ) %>% kable_styling(full_width = F, position = "left",bootstrap_options = c( "condensed"), font_size = 11 ) %>%  pack_rows("Acidobacteria", start_row = 1, end_row = 2) %>% pack_rows("Actinobacteria", 3, 4) %>% pack_rows("Bacteroidetes", 5, 9) %>% pack_rows("Chloroflexi ", 10, 12) %>% pack_rows("Planctomycetes ", 13, 14)  %>%  add_header_above(c(" " = 3,"Chromosol" = 2,"Kurosol" = 2 ))
...
options(knitr.kable.NA = '')
kable(p1,  format = "html", caption = "Number of ASVs in taxonomic groups which had a higher chance to be present in samples with high dieldrin loss than could be explained by chance. Here only samples in the Kurosol were analysed to identify taxa-associations to dieldrin loss specific to the this soil.", digits = 2 ) %>% kable_styling(full_width = F, position = "left",bootstrap_options = c( "condensed"), font_size = 11 ) %>%  pack_rows("Actinobacteria", start_row = 1, end_row = 6) %>% pack_rows("Bacteroidetes", 7, 11) %>% pack_rows("Chloroflexi", 12, 17) %>% pack_rows("Firmicutes", 18, 21) %>% pack_rows("Proteobacteria", 22, 23) %>%   add_header_above(c(" " = 3,"Chromosol" = 2,"Kurosol" = 2 ))
...
#p2.1 <- physeqB.melt.comp %>% dplyr::filter(OTU %in% pf.list$OTU) %>% filter(Abundance > 0)  %>% dplyr::group_by(Phylum, Dloss_fct2) 
#p2.1 <-p2.1 %>% dplyr::summarize(Abun. = mean(Abundance), se = std(Abundance))
#p2.1 <- p2.1 %>% as.matrix %>% as.data.frame() 
#remove "p__"...
#p2 <- p2 %>%  mutate(Phylum = str_replace_all(Phylum, pattern = c(' p__' = "")))  %>%  mutate(Class = str_replace_all(Class, pattern = c(' c__' = "")))  %>%  mutate(Order = str_replace_all(Order, pattern = c(' o__' = "")))
#p2.1 <- p2.1 %>%  mutate(Phylum = str_replace_all(Phylum, pattern = c(' p__' = "")))
#p2.1 <- p2.1 %>%  tidyr::pivot_wider(names_from = Dloss_fct2, values_from = c(Abun., se) )
#p2 <-  dplyr::bind_rows(p2, p2.1) %>% arrange(Phylum, !is.na(Order), `Abun._>=72%`)
#p2[1,"Phylum"] <- "Actinobacteria Total"
#p2[7,"Phylum"] <- "Bacteroidetes Total"
#p2[12,"Phylum"] <- "Chloroflexi Total"
#p2[18,"Phylum"] <- "Firmicutes Total"
#p2[22,"Phylum"] <- "Proteobacteria Total"
#p2[,c(4:11)] <-  p2[,c(4:11)]  %>% mutate_if(is.character, as.numeric)
#p2 <- p2 %>% dplyr::rename(`Abun.1` = `Abun._<=43%`, `Abun.2` = `Abun._>43%`, 
#                           `Abun.3` = `Abun._<72%`, `Abun.4` =  `Abun._>=72%` , 
#                           `se1` = `se_<=43%`, `se2` = `se_>43%`,`se3` = `se_<72%`,`se4` = `se_>=72%`)
...
#kable(p2, format = "html", caption = "Mean abundances of taxonomic groups which had a higher chance to be present in samples with high dieldrin loss than could be explained by chance. Only samples in the Kurosol were analysed to identify Kurosol specific taxa", digits = 1) %>% kable_styling(full_width = F, position = "left",bootstrap_options = c( "condensed"), font_size = 11) %>%  pack_rows("Actinobacteria", start_row = 1, end_row = 6) %>% pack_rows("Bacteroidetes", 7, 11) %>% pack_rows("Chloroflexi", 12, 17) %>% pack_rows("Firmicutes", 18, 21) %>% pack_rows("Proteobacteria", 22, 23) %>%  add_header_above(c(" " = 3,"<=43%" = 2,">43%" = 2,"<72%" = 2,">=72%" = 2))  %>%  add_header_above(c(" " = 3,"Chromosol" = 4,"Kurosol" = 4 ))
