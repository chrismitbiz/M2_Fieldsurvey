...
```{r plsdaallNOTINCLUDE, fig.cap = "Across all 36 samples: Potentials of metabolic pathways which were predicted to be different between samples with low and high dieldrin loss (%) by partial least squares discriminant anaylysis (PLS-DA). Samples are grouped by long-term dielrin loss (%) where the loss is either below or above the median loss in each soil type. Significance of global Kruskal-Wallis tests and Wilcoxon tests for each soil type are shown.", message=FALSE, warning=FALSE, echo=FALSE, fig.width=15, fig.asp = 0.5, cache = TRUE,  include= FALSE, eval=FALSE}
...
```{r plsdatableall, message=FALSE, warning=FALSE, echo=FALSE, cache = TRUE}
...
test1 <- test %>% dplyr::select(pathway,Dloss.dim1,Dloss.dim2) %>% filter(Dloss.dim1 != 0 | Dloss.dim2 != 0 ) %>% arrange(desc(Dloss.dim1)) %>% filter(Dloss.dim1 >= 0.0005 | Dloss.dim1 <= -0.012)


kable(test1, booktabs = T,  digits = 3, col.names = c("Pathway", "Dim1", "Dim2"), caption = "Top regression coefficients for dieldrin of sPLS analysis across both soils (n=33). Values are the predicted effect of metabolic pathway potentials on dieldrin concentrations (table not formated)")
```
...
Among 410 Metacyc pathways included in sPLS regression and sPLS-DA, the genentic potential the genetic potentials for fucose, rhamnose and glycol degradation was most predictive for low dieldrin concentrations (**Table x**, Fig. \@ref(fig:splskurosol)) and high dieldrin loss  (Fig. \@ref(fig:plsdaratiocomp1to3)).  
  
The two soils had in common that positive pathway associations to dieldrin loss coincided with associations to lower POC (Fig. \@ref(fig:plsheatmaps)).
