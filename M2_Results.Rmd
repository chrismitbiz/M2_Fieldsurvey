...
kable(metadata2[c(1:5,33:36),c("Soil","Paddock", "Farm", "Dloss", "D1517", "ROC", "HUM", "POC")], booktabs=TRUE) %>% kable_styling(latex_options="scale_down")
```

## Bacteria
```{r physeqb object,include=TRUE, echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE}
#SVs <-read_qza("~/Documents/Study/LaTrobe/Research/phD/Milestone2_Field_Survey/M2_Bacteria/M2_Qiime_output_Bacteria2020run_200317/feature_table.qza")
SVs <-read_qza("~/Documents/Study/LaTrobe/Research/phD/Milestone2_Field_Survey/M2_Bacteria/M2_Qiime_output_Bacteria2020run_200317/feature_table_insertiontreefiltered.qza")
taxonomy <- read_tsv("/Users/christiankrohn/Documents/Study/LaTrobe/Research/phD/Milestone2_Field_Survey/M2_Bacteria/M2_Qiime_output_Bacteria2020run_200317/taxonomy_silva.tsv") %>% filter(Taxon != "categorical")  %>% rename(Feature.ID = `Feature ID`)
...
tree <- read_qza("~/Documents/Study/LaTrobe/Research/phD/Milestone2_Field_Survey/M2_Bacteria/M2_Qiime_output_Bacteria2020run_200317/insertion-tree.qza")
...
print(paste("The minimum and maximum number of quality bacterial sequences before filtering (per sample) in this data set is",min(colSums(otu_table(physeqB))),"and", max(colSums(otu_table(physeqB))),". After filering this changed to", min(colSums(otu_table(physeqB.flt))),"and",max(colSums(otu_table(physeqB.flt))) ))
...
p1 <- sample_data(physeqB) %>% ggbarplot(., x = "Dloss_fct2", y = "Observed", add = c("mean_se"), scales = "free",fill = "Soil", ylab = "ASV number", xlab = "Dieldrin loss",position = position_dodge(0.9), width = 0.6 ) + stat_compare_means(comparisons = my_comparisons,label = "p.signif", label.y = c(1800)) + stat_compare_means( method = "kruskal",label = "p.signif", label.y = c(1850)) + theme_bw() 
...
p3 <- sample_data(physeqB) %>% ggbarplot(., x = "Dloss_fct2", y = "InvSimpson", add = c("mean_se"), scales = "free",fill = "Soil", ylab = "Simpson index", xlab = "Dieldrin loss",position = position_dodge(0.9), width = 0.6 ) + stat_compare_means(comparisons = my_comparisons,label = "p.signif", label.y = c(450)) + stat_compare_means( method = "kruskal",label = "p.signif", label.y = c(470)) + theme_bw() 
...
p7 <- sample_data(physeqF) %>% ggbarplot(., x = "Dloss_fct2", y = "InvSimpson", add = c("mean_se"), scales = "free",fill = "Soil", ylab = "Simpson index", xlab = "Dieldrin loss",position = position_dodge(0.9), width = 0.6 ) + stat_compare_means(comparisons = my_comparisons,label = "p.signif", label.y = c(50)) + stat_compare_means( method = "kruskal",label = "p.signif", label.y = c(55)) + theme_bw() 
p8 <- sample_data(physeqF) %>% ggbarplot(., x = "Dloss_fct2", y = "pielou_ev", add = c("mean_se"), scales = "free",fill = "Soil", ylab ="Pielou evenness", xlab = "Dieldrin loss",position = position_dodge(0.9), width = 0.6 ) + stat_compare_means(comparisons = my_comparisons,label = "p.signif", label.y = c(0.85)) + stat_compare_means( method = "kruskal",label = "p.signif", label.y = c(1)) + theme_bw() 
...
ASV richness of bacteria and fungi were greater in samples with high long-term dieldrin losses (Fig. \@ref(fig:alphadloss)). Bacterial richness was significanly higher in the Kurosol as well as in samples of the Kurosol in which higher long-term dieldrin losses were observed. A higher ASV richness coincided with increases in other diversity indices, especially an increased bacterial Simpson index. In other words, in a high-loss soil there were more ASVs that were differently abundant, particularly the more dominant bacterial ASVs.
...
y$CN <- log1p(y$CN)
y$HC <- log1p(y$HC)
pw.spls <- mixOmics::spls(data.TSS, y, ncomp =3,  mode = "regression", scale = FALSE ,keepX = c(80,80,80),logratio="CLR")

mixOmics::cim(pw.spls, comp = 1:2, xlab = "", title = "Across both soils", threshold = 0.10, margins = c(5, 20), )
