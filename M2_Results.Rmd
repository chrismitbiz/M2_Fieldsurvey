...
Spectra were pre-processed as described in @Madhavan2017 and concentrations of total organic carbon (TOC), particulate organic carbon (POC), resistant organic carbon (ROC) and humus (HOC) were predicted from MNIRS spectra of the Australian Soil Carbon Research Project (ScaRP, Baldock et al. 2013) using partial least squares (PLS) regression. To assess prediction accuracy the TOC concentrations from the PLS predictions were correlated with TC measurements of the same samples using a dry combustion analyzer (Perkin Elmer 2400 Series Ⅱ, R^2^ = 0.98). For all downstream analyses, POC, ROC and HOC were either transformed into percentages of TOC or centered log ratios [@Bruckner2017a]. 
...
## Phylogenetic associations to dieldrin losses 
...
```{r pcoa, echo= FALSE,  fig.cap="Ordinations of Aitchison distances were compared for  compositions of bacteria, their enzyme predictions and fungi (n = 36). All abundances were filtered to those present in at least 50% of samples. Symbols represent samples and their proximity to each other indicates compositional similarity. The colour and size of symbols indicate the long-term dieldrin loss (%) of samples. The shape of symbols indicate soil type.", fig.width=9, fig.asp= 0.4,message=FALSE, warning=FALSE, results=FALSE}

physeq.B.flt = filter_taxa(physeqB, function(x) sum(x > 0) > (0.5*length(x)), TRUE) #must be present in x percent of samples
physeq.B.flt <-  microbiome::transform(physeq.B.flt, "clr") #clr transform

physeq.F.flt = filter_taxa(physeqF, function(x) sum(x > 0) > (0.5*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt <-  microbiome::transform(physeq.F.flt, "clr") #clr transform

physeq.E.flt = filter_taxa(physeqE, function(x) sum(x > 0) > (0.5*length(x)), TRUE) #must be present in x percent of samples
...
p.pcoa3 <- phyloseq::plot_ordination(physeq.B.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Bacteria") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F)
...
p.pcoaF3 <- phyloseq::plot_ordination(physeq.F.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Fungi") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F)
...
p.pcoaE3 <- phyloseq::plot_ordination(physeq.E.flt, ord_pcoa , color = "Dloss", shape = "Soil") + geom_point(aes(size = D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) +  ggtitle("Enzyme metagenome")+  scale_colour_gradient(low = "cadetblue1", high = "navy")+ guides(alpha = F)
...
```{r phylodiv,  fig.cap= "Faith phylogenetic diversity index (a) and principle coordinate analysis (PCoA) with unweighted unifrac distances (b).", echo=FALSE, fig.height=8, fig.asp=0.4, message=FALSE, warning=FALSE}
...
p.pcoa2 <- phyloseq::plot_ordination(rare_Bac, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") +  scale_colour_gradient(low = "cadetblue1", high = "navy" ) + guides(alpha = F) + annotate(geom = 'text', label = "unweighted unifrac", x = -0.15, y = -Inf, hjust = -0.05, vjust = -1)

pdf(NULL)
gg <- ggarrange(p1, p.pcoa2, ncol = 2, widths = c(0.43,0.5), labels = "auto")
x = dev.off()
gg
```

### Redundancy analysis (Aitchison distances)
```{r rda, echo=FALSE,  fig.cap="Redundancy analysis of bacterial and fungal abundances using the proportions of resistant organic carbon (ROC), humus (HUM) and particulate organic carbon (POC) as constraining variables. ASVs were filtered and the abundances were transformed to Aitchison distances prior to analysis. Symbols represent samples and their proximity to each other indicates compositional similarity. The size and shape of symbols indicate the long-term dieldrin loss (%) of samples. Three samples from the Kurosol were removed as outliers (Kurosol n = 18, Chromosol n = 15)", fig.width=7.5, fig.asp= 0.8,message=FALSE, warning=FALSE}
...
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss") + geom_point(aes(size =  Dloss)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Bacteria, Kurosol") 
...
p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss") +
    geom_point(aes(size =  Dloss)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Fungi, Kurosol") 
...
p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss", shape = "Soil") + geom_point(aes(size = Dloss)) + scale_size(breaks = c(30,40,50)) + scale_colour_gradient(low = "cadetblue1", high = "navy") +
  ggtitle("Fungi, Chromosol") + scale_shape_manual(values = c(17))
