...
- The soil better able to degrade dieldrin (Kurosol)...
...
test1 <- test %>% dplyr::select(pathway, D1517.dim1,D1517.dim2,D1517.dim3) %>% filter(D1517.dim1 != 0 | D1517.dim2 != 0 ) %>% arrange(D1517.dim1) %>% filter(D1517.dim1 >= 0.003 | D1517.dim1 <= -0.01)
...
```{r splskurosol, message=FALSE, warning=FALSE, echo=FALSE, fig.width=10, fig.asp = 0.6, results = FALSE, cache = TRUE}
...
#tables for analysis
x <- as.data.frame(t(otu_table(physeq.pw.flt.clr)))
y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM", "ROC", "POC", "Dloss", "H")])
#y$D1517 <- y$D1517 / 1000 #everything on same units (g/kg) 
#relative data
require(compositions)
y <- data.frame(clr(y))
...
pw.spls <- mixOmics::spls(x, y, ncomp =5,  mode = "regression", scale = FALSE, keepX = c(250,25))
...
pw.spls <- mixOmics::spls(x, y, ncomp =5,  mode = "regression", scale = FALSE, keepX = c(400,10))
#tune.spls <- mixOmics::perf(pw.spls, validation = "Mfold", folds = 10, progressBar = FALSE, nrepeat = 1000)
#plot(tune.spls$Q2.total,main = "sPLS Q^2 Values by Principal Component")
#abline(h = 0.0975)

mixOmics::cim(pw.spls, comp = 1, xlab = "", title = "Chromosol", margins = c(5, 25))
...
Among 410 Metacyc pathways included in sPLS regression and sPLS-DA, the genentic potential for oxidative glucose degradation was most predictive for both, high dieldrin concentrations (**Table x**, Fig. S\@ref(fig:plsheatmapsall)) and lower dieldrin loss (Fig. \@ref(fig:plsdaratiocomp1to3)). On the other hand, the genetic potentials for fucose, rhamnose and glycol degradation was most predictive for low dieldrin concentrations (**Table x**, Fig. S\@ref(fig:plsheatmapsall)) and high dieldrin loss  (Fig. \@ref(fig:plsdaratiocomp1to3)).  
  
Compared to the Chromosol, the Kurosol had a higher proportion of pathways that were associated to greater dieldrin losses (46% of pathways with positive regression coefficients compared to 28%) (Fig. \@ref(fig:plsheatmaps)). The two soils had in common that positive pathway associations to dieldrin loss coincided with associations to lower POC (Fig. \@ref(fig:plsheatmaps)).
...
mixOmics::cim(pw.spls, comp = 1:2, xlab = "", title = "Across both soils", margins = c(5, 30), threshold = 0.3)
