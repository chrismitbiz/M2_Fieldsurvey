---
title: "PrepreDRAFT: A preliminary field survey indicated that carbon quality is a driver for the metabolic capacity of soils to biodegrade persistant organic pollutants"
...
It was found that both surface soils consisted of a similar texture which were classified as loam [@InternationalSocietyofSoilScienceISSS1929]. Furthermore, they had the same average pH (4.55 ± 0.04 SE) and were both in high-rainfall zones with similar annual maximum temperatures (20.9 - 21.9 ˚C). However, they differed in total carbon concentrations, in that the Kurosol measured just over half the carbon concentrations compared to the Chromosol (33.3 and 61.8 mg g^-1^, ± 1.05 and 1.72 SE).
  
The long-term dieldrin loss in percent per paddock was calculated using the difference of the average dieldrin concentrations from 2015/2017 (D1517) and the earliest available concentrations from 1987 to 1995. @Krohn2019 showed that the two soils differed in their apparent ability to degrade dieldrin. The Kurosol lost 1.7 times more dieldrin compared to the Chromosol (73 and 42 %, ± 0.07 and 0.04 SE) since 1988 and it was found that the total carbon, the carbon-to-nitrogen ratio (C:N), the microbial-carbon-to-total-carbon ratio (MBC:TC) and soil diversity were associated with dieldrin concentrations and dieldrin loss.
...
After over 30 years of its last use, aged dieldrin is unlikely to have significant impact on bacterial abundances. Hence, we aimed to understand if the presence (not abundance) of phylogenetic clades can be associated with a soil environment that was conducive to greater long-term dieldrin losses. Thus, to test whether the presence of some bacterial clades was more likely in a soil with high or low dieldrin loss the package Phylofactor [@Washburne2019a] was used to perform generalized phylofactorization. Phylofactorization was chosen as it has the ability to detect similar shifts in microbial clades among different data (or soils) when mapped to the same phylogeny, even with minimal ASV overlap.  
...
Microbial richness was greater in samples with high long-term dieldrin losses (Fig. \@ref(fig:alphadloss)a, b and c). For example, bacterial richness and their predicted enzyme abundances were highest in samples with highest dieldrin loss (>=72% dieldrin loss in the Kurosol). Fungal richness also trended higher in high-loss in samples within each soil. 
  
A higher ASV richness coincided with increases in other diversity indices, especially an increased bacterial Simpson's index which was 1.27 times greater in the Kurosol. Consequently, in a high-loss environment there were more ASVs that were differently abundant, particularly the more dominant bacterial ASVs.  
    
Interestingly, the diversity (Shannon's, Simpson's and Pilou's evenness) of the enzyme metagenome which was predicted from bacterial ASVs was smaller in the Kurosol compared to the Chromosol which is in contrast to the bacterial diversity itself (Fig. \@ref(fig:alphadloss)c).

Soil diversity was further compared on a per-unit-dieldrin (diversity µg g^-2^) basis (Fig \@ref(fig:alphaperD17)). It showed that the Kurosol, which contained less organic matter and dieldrin, had significantly higher diversity for each µg/g of dieldrin compared to the Chromosol. Hence, there was greater diversity for each molecule of dieldrin in the high-loss environment, including the functional diversity based on the enzyme metagenome.
...
p.pcoa3 <- phyloseq::plot_ordination(physeq.B.flt, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Bacteria") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)", shape = "") 
...
p.pcoaF3 <- phyloseq::plot_ordination(physeq.F.flt, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") + geom_point(aes(size =  D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) + ggtitle("Fungi") +  scale_colour_gradient(low = "cadetblue1", high = "navy") + guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)", shape = "") 
...
p.pcoaE3 <- phyloseq::plot_ordination(physeq.E.flt, ord_pcoa , color = "Dloss", shape = "Dloss_fct2") + geom_point(aes(size = D1517)) + scale_size(breaks = c(0.1,0.5,1.5)) +  ggtitle("Enzyme metagenome")+  scale_colour_gradient(low = "cadetblue1", high = "navy")+ guides(alpha = F) + labs(size = "Dieldrin (µg/g)", color = "Dieldrin loss (%)", shape = "") 
...
### Test for compositional differences (Permanova and betadisp, Bray-Curtis)
#### Betadisp
```{r betadisp, echo= FALSE,message=FALSE, warning=FALSE, results=TRUE}
...
#### Permanova
```{r permanova, echo= FALSE,message=FALSE, warning=FALSE, results=TRUE}
#PERMANOVA
## Bray-Curtis distances between samples
physeq.B.flt  <-  prune_samples(sample_data(rare_Bac)$Farm != "C", rare_Bac)
physeq.B.flt = filter_taxa(physeq.B.flt, function(x) sum(x > 0) > (0.8*length(x)), TRUE) 
...
### Redundancy analysis (Aitchison distances)
...
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss") + geom_point(aes(size =  Dloss)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Bacteria, Kurosol") + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss") +
    geom_point(aes(size =  Dloss)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Fungi, Kurosol") + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss", shape = "Soil") + geom_point(aes(size = Dloss)) + scale_size(breaks = c(30,40,50)) + scale_colour_gradient(low = "cadetblue1", high = "navy") +
  ggtitle("Bacteria, Chromosol") + scale_shape_manual(values = c(17)) + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss", shape = "Soil") + geom_point(aes(size = Dloss)) + scale_size(breaks = c(30,40,50)) + scale_colour_gradient(low = "cadetblue1", high = "navy") +
  ggtitle("Fungi, Chromosol") + scale_shape_manual(values = c(17)) + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
p.ccaE  <- phyloseq::plot_ordination(physeq.E.flt, ord_ccaE , color = "Dloss") +
    geom_point(aes(size =  Dloss)) + scale_colour_gradient(low = "cadetblue1", high = "navy") + ggtitle("Enzyme metagenome, Kurosol") + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
p.ccaE  <- phyloseq::plot_ordination(physeq.E.flt, ord_ccaE , color = "Dloss", shape = "Soil") + geom_point(aes(size = Dloss)) + scale_size(breaks = c(30,40,50)) + scale_colour_gradient(low = "cadetblue1", high = "navy") +
  ggtitle("Enzyme metagenome, Chromosol") + scale_shape_manual(values = c(17)) + labs(size= "Dieldrin loss (%)", color = "Dieldrin loss (%)") 
...
The mean Faith's phylogenetic diversity was similiar between soils and groups of low and high dieldrin loss (Kruskal-Wallis *p* > 0.05) (Fig. \@ref(fig:phylodiv)a). However phylogentic variation trended higher in samples where dieldrin losses were high (Levene's test, *p* = 0.069). Phylogenetic dispersion was also apparent from unifrac distances in figure \@ref(fig:phylodiv)b which showed spreading of samples with high dieldrin loss compared to a tight clustering of groups samples with low dieldrin loss. 

We further did RDA, to explore how ROC, HUM and POC was associated with microbial community compostion and dieldrin. Only in the Kurosol did carbon fractions significantly associate with the composition of bacteria, the enzyme metagenome and fungi (Fig \@ref(fig:rda)). Permutation tests for the joint effect of ROC, HUM and POC were significant for the Kurosol (*p* < 0.001) but not for the Chromosol (*p* > 0.05). The variation of carbon fractions in the Kurosol therefore explained 29.5 and 29.4 % of the compositional variation of bacteria and fungi respectively (RDA1 and RDA2 combined), while no effect on the microbial composition in the Chromosol could be detected.  
...
```{r plsheatmapsku, fig.cap = "Ward clustered heatmap of similarity scores obtained from sparse partial least squares analysis in regression mode (R package mixOmics) for pathway potentials (rows) and soil variables (columns) for Kurosol samples (n = 18). Soil variables included the average dieldrin concentrations from 2015 and 2017 (log D1517), C:N ratio (log CN), H:C ratio (log HC), ROC, HUM and POC. The pathway potentials and ROC, HUM and POC were clr transformed before analysis.", message=FALSE, warning=FALSE, echo=FALSE, fig.width=10, fig.asp = 0.6, results = FALSE, cache = TRUE}
...
Note: Need to clarify significance calculations. Deviance is huge but nonetheless seems to match the high density of presence of unique ASVs in outer ring. 
...
Figure \@ref(fig:trees) reconfirmed that the phylogenetic composition of the Kurosol and the Chromosol was similar (as observed with Faith PD diversity index,  fig. \@ref(fig:phylodiv)a). However, the number of red lines in the outer rings show that the Kurosol contained more unique ASVs in high-loss samples. Per sample, the Kurosol contained 48 unique ASVs on average compared to 21 in the Chromosol. This  observation is in line with a greater phylogenetic dispersion seen in figure \@ref(fig:phylodiv)b and implied a greater phylogenetic versatility in samples with greater dieldrin degradation. 
...
physeq.B.flt = filter_taxa(physeq.B.flt , function(x) sum(x > 0) > (0.30*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.15*length(x)), TRUE) #must be present in x percent of samples
physeq.B.flt
min(taxa_sums(physeq.B.flt))
physeq.F.flt
...
#saveRDS(sse.physeqBF, file = "~/Documents/Study/LaTrobe/Research/phD/Milestone2_Field_Survey/M2_Bacteria/R_FieldSurvey16s_M2/sse.physeqBF_KUROSOL_26March20")
#sse.physeqBFK <- readRDS("/Volumes/Backup Plus/ManualFileBackup/SpiecEasiFiles/sse.physeqBF_KUROSOL_26March20")
...
# Acknowledgements
Josh Vido, Helen Hayden, Matt Kitching (check a), Gene Drendle, Steven Batinovic, Steve Petrovski, Jen Wood, Sarah Knowler
