...
## Metabolic potential
### Comparison of chlorinated compound degradation potentials
```{r xenobgraphs, fig.cap = "Comparison of all xenobiotic degradation pathways predicted to be present in all samples. Samples are grouped by long-term dielrin loss (%) where the loss is either below or above the median loss in each soil type. Significance of global Kruskal-Wallis tests (topleft) and Wilcoxon tests (above each group) for each soil type are shown.", message=FALSE, warning=FALSE, echo=FALSE, fig.width=9, fig.asp = 0.45, cache = TRUE}
...
# ratio ####
TC <- sample_data(physeq.pw.flt) %>% data.frame %>% rownames_to_column("OTUID") %>% dplyr::select(OTUID, TC)
data.x.TC <- x %>% as.data.frame %>%  rownames_to_column("OTUID") %>% left_join(TC) %>% column_to_rownames("OTUID")
#then devide by TC or Dieldrin and remove TC or dieldrin
data.x.TC.ratio <- data.x.TC %>% rownames_to_column("OTUID") %>% mutate_at(vars(- TC, - OTUID), ~ . / TC) %>% column_to_rownames("OTUID") %>% dplyr::select(-TC)

data.x.TC.ratio.phyl <- data.x.TC.ratio %>% t 
physeq.pw_TCratio <-phyloseq(otu_table(data.x.TC.ratio.phyl, taxa_are_rows = T),   tax_table(as.data.frame(pw.names) %>% as.matrix()), sample_data(metadata2 %>% as.data.frame() %>% column_to_rownames("#SampleID"))) 
ps_clr <- microbiome::transform(physeq.pw_TCratio, "clr")
...
test1 <- test %>% dplyr::select(pathway, D1517.dim1,D1517.dim2 ) %>% filter(D1517.dim1 != 0 | D1517.dim2 != 0 ) %>% arrange(D1517.dim1) %>% filter(D1517.dim1 >= 0.003 | D1517.dim1 <= -0.01)
...
pred <- predict(pw.spls, as.matrix(x))
test <- data.frame(pred$B.hat) %>% rownames_to_column("pathway")
test2 <- test %>% dplyr::select(pathway, D1517.dim1,D1517.dim2 ) %>% filter(D1517.dim1 != 0 | D1517.dim2 != 0 ) %>% arrange(D1517.dim1)
...
Only 1 of the 44 chlorinated compound degradation pathways available in the Metacyc database (including the xenobiotics lindane and pentachlorophenol) was detected. Chlorosalicylate degradation was the only genetic potential found that was related to xenobiotic chlorinated compound degradation and these metabolic potentials were significantly greater in samples with high dieldrin loss (Fig. \@ref(fig:xenobgraphs)). Chlorosalicylates are chloroaromatics and active ingredients or additives of antimicrobial products and pesticides (reference).    
  
Furthermore figure \@ref(fig:xenobgraphs) shows there was an increased mean metabolic potential of 39 aromatic compound degradation pathways in the Kurosol. 

Figure \@ref(fig:plsheatmapsall), \@ref(fig:plsheatmapsku), \@ref(fig:plsheatmapsch) and \@ref(fig:plsdaratiocomp1to3).
...
physeq.B.flt = filter_taxa(physeq.B.flt , function(x) sum(x > 0) > (0.3*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.25*length(x)), TRUE) #must be present in x percent of samples
physeq.B.flt
min(taxa_sums(physeq.B.flt))
physeq.F.flt
