...
#y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM.pct", "ROC.pct", "POC.pct", "D1517", "CN", "HC")]) 
y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM", "ROC", "POC", "D1517", "TN", "H")])
...
pw.spls <- mixOmics::spls(data.TSS, y, ncomp =5,  mode = "regression", scale = FALSE, keepX = c(5,10,20), logratio = "CLR")
...
mixOmics::cim(pw.spls, comp = 1:3, xlab = "", title = "Across both soils", margins = c(5, 30))
...
pw.spls <- mixOmics::spls(data.TSS, y, ncomp =10,  mode = "regression", scale = FALSE, keepX = c(5,10,5,5), logratio = "CLR")
tune.spls <- mixOmics::perf(pw.spls, validation = "Mfold", folds = 10, progressBar = FALSE, nrepeat = 500)
plot(tune.spls$Q2.total,main = "sPLS Q^2 Values by Principal Component")
abline(h = 0.0975)

mixOmics::cim(pw.spls, comp = 1, xlab = "", title = "Kurosol", margins = c(5, 25))
...
#y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM.pct", "ROC.pct", "POC.pct", "D1517", "CN", "HC")]) #use 
...
Note: The following pathways were selected by PLS-DA
...
#dloss.plsda <- splsda(data.TSS.TC.ratio, y, ncomp = 5, logratio = "CLR")  # set ncomp to 10 for performance assessment later
...
splsda.dloss2 <- splsda(data.TSS.TC.ratio, y, ncomp = ncomp, keepX = select.keepX, logratio = "CLR") 
...
data.TSS.TC.ratio <- data.TSS.TC %>% rownames_to_column("OTUID") %>% 
  mutate_at(vars(- TC, - OTUID), ~ . / TC) %>% column_to_rownames("OTUID") %>% dplyr::select(-TC)
data.TSS.TC.ratio <- log10(data.TSS.TC.ratio)
