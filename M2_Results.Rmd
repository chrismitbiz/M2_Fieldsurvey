---
title: "DRAFT: A preliminary field survey indicated that carbon quality is a driver for the metabolic capacity of soils to biodegrade persistant organic pollutants"
...
p.ccaB  <- phyloseq::plot_ordination(physeq.B.flt, ord_ccaB , color = "Dloss", shape = "Dloss_fct2") +
    geom_point(aes(size =  Dloss)) +
   scale_colour_gradient(low = "cadetblue1", high = "navy")+
  ggtitle("Bacteria, Kurosol") 
...
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.2*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt <-  microbiome::transform(physeq.F.flt, "clr") #clr transform
...
p.ccaB  <- phyloseq::plot_ordination(physeq.F.flt, ord_ccaF , color = "Dloss", shape = "Dloss_fct2") +
    geom_point(aes(size =  Dloss)) +
    scale_colour_gradient(low = "cadetblue1", high = "navy")+
  ggtitle("Fungi, Kurosol") 
...
physeq.F.flt = filter_taxa(physeq.F.flt, function(x) sum(x > 0) > (0.20*length(x)), TRUE) #must be present in x percent of samples
physeq.F.flt <-  microbiome::transform(physeq.F.flt, "clr") #clr transform
...
physeq.E.flt = filter_taxa(physeq.E.flt, function(x) sum(x > 0) > (0.2*length(x)), TRUE) #must be present in x percent of samples
physeq.E.flt <-  microbiome::transform(physeq.E.flt, "clr") #clr transform
