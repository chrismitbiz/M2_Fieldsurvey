...
```{r physeqb object,include=FALSE, echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE}
...
```{r physeqf object,include=FALSE, echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE}
#FUNGI
...
```{r physeqE object,include=FALSE, echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE}
...
```{r physeqpw object, include=FALSE, echo=FALSE, warning=FALSE,message=FALSE, cache=TRUE}
...
Subsequent null-model comparisons to each ROC, HUM and ROC individually indicated that HUM was most important for the community composition, followed by ROC. In the Kurosol, there was some grouping of samples in which the composition of bacteria and fungi associated with ROC which coincided with the lowest long-term dieldrin loss (%). On the other hand, the highest dieldrin loss (%) was observed in samples where the community composition associated with HUM. POC was insignificant in the Kurosol RDA which was also indicated by a short arrow in figure \@ref(fig:rda). 
...
```{r plsheatmapsku, message=FALSE, warning=FALSE, echo=FALSE, fig.width=10, fig.asp = 0.6, results = FALSE, cache = TRUE}
...
#kable(test2, booktabs=TRUE, caption = "Regression coefficients of sPLS analysis across in the Kurosol (n=18). Values are the predicted effect of the metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")
...
```{r plsheatmapsch,fig.cap = "Ward clustered heatmap of similarity scores obtained from sparse partial least squares analysis in regression mode (R package mixOmics) for pathway potentials (rows) and soil variables (columns) for Kurosol (top) and Chromosol (bottom) samples (n = 15). Soil variabels included the average dieldrin concentrations from 2015 and 2017 (log D1517), C:N ratio (log CN), H:C ratio (log HC), ROC, HUM and POC. The pathway potentials and ROC, HUM and POC were clr transformed before analysis.", message=FALSE, warning=FALSE,echo=FALSE, fig.width=10, fig.asp = 0.6, cache = TRUE }
...
#kable(test3, booktabs=TRUE, caption = "Top regression coefficients of sPLS analysis across in the Chromosol (n=15). Values are the predicted effect of the metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")
...
**Metabolic potential results summary**  
Only 1 of the 44 chlorinated compound degradation pathways available in the Metacyc database (including the xenobiotics lindane and pentachlorophenol which are also organochlorines) was detected. Chlorosalicylate degradation was the only genetic potential found that was related to xenobiotic chlorinated compound degradation and these metabolic potentials were significantly greater in samples with high dieldrin loss (Fig. \@ref(fig:xenobgraphs)). Chlorosalicylates are chloroaromatics and active ingredients or additives of antimicrobial products and pesticides (reference).    
  
Furthermore figure \@ref(fig:xenobgraphs) shows that the mean metabolic potential of aromatic compound degradation was greater in the Kurosol and greatest in the samples with highest dieldrin losses. 

Among 410 Metacyc pathways included in sPLS regression and sPLS-DA, the genentic potential for oxidative glucose degradation was most predictive for both, high dieldrin concentrations (Fig. \@ref(fig:plsheatmapsall) and table 4) and lower dieldrin loss (Fig. \@ref(fig:plsdaratiocomp1to3)). On the other hand, the genetic potentials for fucose, rhamnose and glycol degradation was most predictive for low dieldrin concentrations (Fig. \@ref(fig:plsheatmapsall) and table 4) and high dieldrin loss  (Fig. \@ref(fig:plsdaratiocomp1to3)).  
  
Furthermore, from figure \@ref(fig:plsheatmapsall) it was apparent that across both soils the metabolic potentials that were associated with low dieldrin concentrations were also associated to POC, HUM and H but not to ROC.   
  
Compared to the Chromosol, the Kurosol had a higher proportion of pathways that were associated to greater dieldrin losses (46% of pathways with positive regression coefficients compared to 28%) (Fig. \@ref(fig:plsheatmapsku) and  \@ref(fig:plsheatmapsch)). Furthermore, the co-associations of metabolic potentials to carbon fraction and dieldrin losses were reversed in the Kurosol and the Chromosol. In the Kurosol, the metabolic potentials that were associated to greater dieldrin losses were also associated to more H, HUM and ROC while in the Chromosol these pathways were co-associated with lower dieldrin losses.  

<br/>

## Phylogenetic analysis
```{r treesnotinclude,fig.cap = "Comparison of bacterial phylogeny present in the Kurosol and the Chromosol (a). Tips represent individual ASVs and their colours indicate their phylum membership. Unique ASVs are shown in the outer ring where red indicates if ASVs are unique to a 'high-loss' environment (High-loss = above median dieldrin loss (%)). Three outlier samples were removed prior to analysis (Kurosol n = 18, Chromosol n = 15). Polygons highlight phylogenetic clades with a higher chance of presence in high-loss samples than could be explained by chance. Clades and associated ASV abundances are further shown (b). Here samples were grouped into four factors by long-term dielrin loss (%) where the loss is either below or above the median dieldrin loss in each soil type (Median loss was 43% and 72% in the Chromosol and Kurosol respectively). Prior to analysis ASVs were filted to those with a minimum of 10 reads.", message=FALSE, warning=FALSE,echo=FALSE, fig.width=12, fig.asp = 0.75, cache = TRUE }
library(ggtree)
library(ggnewscale)
...
p3 <- ggarrange(p1.1, p2.2, ncol = 2, common.legend = TRUE)

p4 <- ggarrange(p3, p1, nrow = 2, labels = "auto")
...
Note: Need to clarify significance calculations. Deviance is huge but nonetheless seems to be significant and match the high density of presence of unique ASVs in outer ring. 

<br/>

Figure \@ref(fig:trees) affirmed that the phylogenetic composition of the Kurosol and the Chromosol was similar which was also observed with the Faith PD diversity index,  fig. S\@ref(fig:phylodiv)a). However, the number of red lines in the outer rings show that the Kurosol contained more unique ASVs in high-loss samples. Per sample, the Kurosol contained 48 unique ASVs on average compared to 21 in the Chromosol. This  observation is in line with a greater phylogenetic dispersion seen in figure S\@ref(fig:phylodiv)b and implied a greater phylogenetic versatility in samples with greater dieldrin degradation. 
  
Moreover, generalised linear models (glm) in the R package 'phylofactor', with presence/absence of ASVs as response and dieldrin loss (%) as predictor, indicated that there were phylogentic clades that had a significantly higher chance to be found in 'high-loss' samples (shown as polygons on phylogenetic tree in figure  \@ref(fig:trees)). In some instances, these phylogenetic clades coincided with higher clustering of unique ASVs. This meant that genetic versatility was greater in samples were greater dieldrin degradation was observed.  
  
For example the presence of clades of Firmicutes, Bacteriodetes, Chloroflexi and Actinobacteria were associated with increased dieldrin losses (Fig. \@ref(fig:pfmixall) and \@ref(fig:pfmixKU)). Moreover, a group of ASVs were identified that belong to genus *Geobacter* (order Desulfuromonadales), which were overrepresented in samples with highest dieldrin losses (Fig \@ref(fig:pfmixKU)). It was observed that all of these clades contained more and more different ASVs in 'high-loss' samples. Mean abundances of these ASVs were not associated with dielrin loss. 
  
In the Chromosol, the presence of a clade of Acidobacteria and Actinobacteria was associated with dieldrin loss (Fig \@ref(fig:pfmixCH)). 
...
```{r glassomodulecor, fig.cap = "Scatterplot of the normalised abundances of the combined taxa in module zero from the network analysis of the Kurosol.",  message=FALSE, warning=FALSE, echo=FALSE, fig.width=3, fig.asp = 1, include=TRUE, cache = TRUE }
...
kable(test1, caption = "Top regression coefficients of sPLS analysis across both soils (n=33). Values are the predicted effect of metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")
```

<br/>

## Sparse Partial Least Squares regressi
