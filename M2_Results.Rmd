...
Note: This section gives an overview of metadata and the data of bacterial and fungal amplicon sequence variants (ASVs) used as basis for all analyses. 
...
#y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM.clr", "ROC.clr", "POC.clr", "Dieldrin", "CN", "HC")])

y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM.pct", "ROC.pct", "POC.pct", "D1517", "CN", "HC")]) #use relative data
require(compositions)
y <- data.frame(clr(y))

pw.spls <- mixOmics::spls(data.TSS, y, ncomp =10,  mode = "regression", scale = FALSE, keepX = c(10,50,20), logratio = "CLR")
tune.spls <- mixOmics::perf(pw.spls, validation = "Mfold", folds = 20, progressBar = FALSE, nrepeat = 1000)
#plot(tune.spls$Q2.total,main = "sPLS Q^2 Values by Principal Component")
#abline(h = 0.0975)

mixOmics::cim(pw.spls, comp = 1:2, xlab = "", title = "Across both soils", margins = c(5, 30))
...
pred <- predict(pw.spls, as.matrix(data.TSS))
test <- data.frame(pred$B.hat) %>% rownames_to_column("pathway")
test <- test %>% dplyr::select(pathway, D1517.dim1,D1517.dim2 ) %>%
  filter(D1517.dim1 != 0 | D1517.dim2 != 0 ) %>% arrange(D1517.dim1)

kable(test, booktabs=TRUE, caption = "Regression coefficients of sPLS analysis across both soils (n=35). Values indicate the predicted effect of the metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")
```
<br/>
...
physeq.pw.flt  <-   prune_samples(sample_data(physeq.pw)$Farm != "C", physeq.pw)
physeq.pw.flt <- prune_samples(sample_data(physeq.pw.flt)$Soil == "Kurosol", physeq.pw.flt)
physeq.pw.flt = filter_taxa(physeq.pw.flt, function(x) sum(x > 0) > (0.99*length(x)), TRUE)
...
pw.spls <- mixOmics::spls(data.TSS, y, ncomp =10,  mode = "regression", scale = FALSE, keepX = c(5,15,15), logratio = "CLR")
#tune.spls <- mixOmics::perf(pw.spls, validation = "Mfold", folds = 10, progressBar = FALSE, nrepeat = 1000)
#plot(tune.spls$Q2.total,main = "sPLS Q^2 Values by Principal Component")
#abline(h = 0.0975)
...
kable(test, booktabs=TRUE, caption = "Regression coefficients of sPLS analysis across in the Kurosol (n=18). Values indicate the predicted effect of the metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")

```

```{r, tblemixomicsku, echo = FALSE, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE }
#print the correlation coordinates
#bisect = pw.spls$variates$X[, 1:2] + pw.spls$variates$Y[, 1:2]
#cord.X = cor(pw.spls$X, bisect, use = "pairwise")
#cord.Y = cor(pw.spls$Y, bisect, use = "pairwise")
...
x <- as.data.frame(t(otu_table(physeq.pw.flt)))
data.TSS = t(apply(x+1, 1, TSS.divide))

#y <- data.frame(sample_data(physeq.pw.flt)[,c("HUM.clr", "ROC.clr", "POC.clr", "D1517","CN", "HC")]) #use relative data
#y$D1517 <- log1p(y$D1517)
#y$CN <- log1p(y$CN)
#y$HC <- log1p(y$HC)
...
pw.spls <- mixOmics::spls(data.TSS, y, ncomp =5,  mode = "regression", scale = FALSE, keepX = c(5,5), logratio = "CLR")
#tune.spls <- mixOmics::perf(pw.spls, validation = "Mfold", folds = 10, progressBar = FALSE, nrepeat = 1000)
#plot(tune.spls$Q2.total,main = "sPLS Q^2 Values by Principal Component")
#abline(h = 0.0975)
...
kable(test, booktabs=TRUE, caption = "Regression coefficients of sPLS analysis across in the Chromosol (n=15). Values indicate the predicted effect of the metabolic pathway potentials on dieldrin concentrations") %>% kable_styling(latex_options="scale_down")
...
colours <- c("grey52", "chocolate","gray","gray", "wheat4",
             "blue4","pink","blueviolet", "aquamarine4", "cadetblue4", "darkgoldenrod4", "darkred","dodgerblue2","tomato1","orange","deeppink","dodgerblue", "darkgrey", "gold", "darkorchid", "darksalmon", "seagreen3", "turquoise3", "yellowgreen","seagreen3", "gray", "orange") # "seagreen3" "darksalmon" "yellowgreen","seagreen3","tomato1""cadetblue4"
...
#tree graphing with colours for phyla
colours <- c("grey52", "chocolate", "gray","gray", "wheat4",
             "pink","blueviolet", "aquamarine4", "cadetblue4", "darkgoldenrod4", "darkred", "dodgerblue2", "tomato1", "orange", "deeppink", "dodgerblue", "darkgrey", "seagreen3", "turquoise3", "yellowgreen", "seagreen3", "tomato1") # "pink", "seagreen3" "darksalmon" "yellowgreen","seagreen3","tomato1"
ggtree_phyla2 <- ggtree(ggtree_gps, aes(color=Phylum,alpha = 0.5), layout = 'circular', branch.length = 'none') +  theme(legend.position="right") + scale_colour_manual(values = colours) + guides(color = guide_legend(override.aes = list(size=5)))
...
```{r pfmixCH, eval=TRUE, include=TRUE, fig.cap = "Result of phylofactor (mixed algorithm) on kurosol (n = 18). Selection of ASVs that had a higher chance to be present in a high loss sample than the remainder of ASVs.", message=FALSE, warning=FALSE,echo=FALSE, fig.width=15, fig.asp = 1, cache = TRUE }
require(metagMisc)
