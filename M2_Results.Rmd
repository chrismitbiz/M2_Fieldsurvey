...
- Beta and phylogenetic diversity were dispersed, and more unique taxa present in high-loss samples indicating that a greater genetic versatility is needed for dieldrin degradation.
- Some groups of bacteria in the order of Acidobacteriales and Solirubrobacteriales which carry the predicted carbohydrate and alcohol degradation genes were also more likely to be present in high-loss’ samples. 
- The presence of five other taxonomic groups (Actinobacteria, Chloroflexi, Planctomycetes, Bacteroidetes and *Geobacter sp.*) was associated with high dieldrin loss and curiously for all of them the genetic potential of aerobic respiration involving cytochrome c was highest in the Picrust2 predictions for these taxons (Link to metacyc pathway [here](https://metacyc.org/META/NEW-IMAGE?type=PATHWAY&object=PWY-3781)). (More literature reading needed here, relating to efficiency of metabolism?)
...
```{r splskurosol, fig.cap = "Ward clustered heatmap of similarity scores obtained from sparse partial least squares analysis in regression mode (R package mixOmics) for pathway potentials (rows) and soil variables (columns) for all samples and for  Kurosol samples only. Soil variabels included the long-term dieldrin loss (%) (Dloss) and concentrations of resistant organic carbon (ROC), humic carbon (HUM), particulate organic carbon (POC) and hydrogen (H). The pathway potentials and ROC, HUM and POC were clr transformed before analysis.", message=FALSE, warning=FALSE, echo=FALSE, out.width="49%", out.height="40%", fig.show='hold', cache = TRUE}
...
knitr::include_graphics(c("./mixomics_all.png","./mixomics_kurosol1.png")  )
...
## Phylogenetic clades that were associated to dieldrin loss (THIS WILL BECOME A TABLE)
