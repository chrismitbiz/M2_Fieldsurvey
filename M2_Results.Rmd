...
physeq.pw.melt2 <- psmelt(physeq.pw.flt.com)
my_comparisons <- list( c("<=43%", ">43%"), c("<72%", ">=72%") )
#preselection 
selection <- splsda_compvec %>% filter(value.var >= 0.3 | value.var <= -0.27)
#figure      
#	'&beta;-(1,4)-mannan degradation'
physeq.pw.melt2  %>% filter(OTU %in% selection$name)  %>% ggboxplot(., x = "Dloss_fct2", y = "Abundance", add = c("jitter"), scales = "free", facet.by = "COMMON.NAME", fill = "CLASS1", ylab = "Metacyc pathway potentials (Relative abundance)", xlab = "Dieldrin loss", nrow = 10, ncol = 4) + stat_compare_means( method = "kruskal",label = "p.signif") + stat_compare_means(comparisons = my_comparisons)
...
test1 <- test %>% dplyr::select(pathway, Dloss.dim1,Dloss.dim2,Dloss.dim3) %>%  arrange(Dloss.dim1) %>% filter(Dloss.dim1 >= 0.0005 | Dloss.dim2 >= 0.0005)
...
Among 410 Metacyc pathways included in sPLS regression and sPLS-DA the genetic potentials for fucose, rhamnose and glycol degradation was most predictive for high dieldrin dissipation across both soils (**Table x**, Fig. \@ref(fig:plsdaratiocomp1to3) and \@ref(fig:splskurosol)). Within the Kurosol samples lactose and galactose degradation was the most strongly associated to more dieldrin dissipation.  
On the other hand oxidative glucose degradatin was associated with samples of low dieldrin disspation (Fig. \@ref(fig:plsdaratiocomp1to3))
